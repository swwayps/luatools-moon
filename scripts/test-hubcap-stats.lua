-- Harness: load the REAL dist main.lua with stubs and exercise GetHubcapStats'
-- failure classification. The Hubcap key-status box must tell a genuinely
...
preload("api_manifest", {
  get_all_apis = function() return { success = true, apis = {} } end,
  get_api_list = function() return { success = true, apis = {} } end,
})
preload("downloads", {})
preload("fixes", {})
...
check(ok_load and type(GetHubcapStats) == "function", "dist main.lua loads and exports GetHubcapStats")
check(type(GetAllApis) == "function" and GetAllApis():find('"apis":[]', 1, true) ~= nil,
  "GetAllApis serializes an empty list as []")
check(type(GetApiList) == "function" and GetApiList():find('"apis":[]', 1, true) ~= nil,
  "GetApiList serializes an empty list as []")

if type(GetHubcapStats) == "function" then
  ...
  local r = GetHubcapStats({ api_key = KEY })
...
if fails == 0 then print("test-hubcap-stats: ALL PASS") else print("test-hubcap-stats: " .. fails .. " FAILURE(S)"); os.exit(1) end
